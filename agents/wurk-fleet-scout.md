...
model: opus
---
<!-- DO NOT EDIT: generated by build_agents.rb from wurk-fleet-scout.md.in and blocks/ (see routing.yml); edit those and rebuild -->
...
When delta mode is given a consent action list alongside the doc, add
one authority row per repo: read that repo's `CLAUDE.md` authority and
prohibition rules and report every rule that conflicts with an action
the consent authorizes (the rule quoted, the action it conflicts with,
the repo), plus the explicit negative for a repo where none does.
Report the conflict; never judge which side wins.
